...
public class SeatFlowControlController extends BaseController {
  ...
  @PreAuthorize("@ss.hasPermi('seatflow:control:checkin')")
  @Operation(summary = "学生提前结束本人正在使用的预约")
  @PostMapping("/complete")
  public AjaxResult complete(@Valid @RequestBody CheckinRequest request) {
    controlService.complete(request.getReservationId(), SecurityUtils.getUserId());
    return AjaxResult.success("已结束使用");
  }

  @PreAuthorize("@ss.hasPermi('seatflow:control:checkin')")
  @Operation(summary = "查询本人管控状态")
  @GetMapping("/profile")
  public AjaxResult profile() {
    return AjaxResult.success(controlService.selectProfile(SecurityUtils.getUserId()));
  }
  ...
  @PreAuthorize("@ss.hasPermi('seatflow:control:blacklist:edit')")
  @Operation(summary = "管理员解除学生黑名单")
  @PutMapping("/blacklist/{blacklistId}/release")
  public AjaxResult releaseBlacklist(@PathVariable Long blacklistId) {
    controlService.releaseBlacklist(blacklistId);
    return AjaxResult.success("已解除黑名单");
  }
}
