...
public interface SeatFlowControlMapper
{
    ...
    List<ControlReservation> selectAvailableCheckins(@Param("userId") Long userId, @Param("now") Date now);
    int markCheckedIn(@Param("reservationId") Long reservationId, @Param("now") Date now);
    int insertCheckin(@Param("reservationId") Long reservationId, @Param("userId") Long userId,
            @Param("now") Date now);
    List<ControlReservation> selectExpiredForUpdate(@Param("now") Date now, @Param("limit") int limit);
    int markNoShow(@Param("reservationId") Long reservationId, @Param("now") Date now);
    int insertViolation(@Param("reservationId") Long reservationId, @Param("userId") Long userId,
    ...
    int insertBlacklist(@Param("userId") Long userId, @Param("violationId") Long violationId,
            @Param("reason") String reason, @Param("now") Date now);
    List<ViolationRecord> selectViolations(@Param("userId") Long userId, @Param("query") ControlQuery query);
