...
public class SeatFlowBaseInfoServiceImpl implements ISeatFlowBaseInfoService {
  ...
  public List<SeatFlowCampus> selectCampusList(SeatFlowCampus q) {
    return mapper.selectCampusList(q);
  }

  public SeatFlowCampus selectCampusById(Long id) {
    return mapper.selectCampusById(id);
  }

  public List<SeatFlowBuilding> selectBuildingList(SeatFlowBuilding q) {
    return mapper.selectBuildingList(q);
  }

  public SeatFlowBuilding selectBuildingById(Long id) {
    return mapper.selectBuildingById(id);
  }

  public List<SeatFlowFloor> selectFloorList(SeatFlowFloor q) {
    return mapper.selectFloorList(q);
  }

  public SeatFlowFloor selectFloorById(Long id) {
    return mapper.selectFloorById(id);
  }

  public List<SeatFlowRoom> selectRoomList(SeatFlowRoom q) {
    return mapper.selectRoomList(q);
  }

  public SeatFlowRoom selectRoomById(Long id) {
    return mapper.selectRoomById(id);
  }

  public List<SeatFlowSeat> selectSeatList(SeatFlowSeat q) {
    return mapper.selectSeatList(q);
  }
  ...
  private void normalize(SeatFlowCampus e) {
    e.setCampusName(e.getCampusName().trim());
    e.setStatus(normalizeStatus(e.getStatus()));
  }

  private void normalize(SeatFlowBuilding e) {
    e.setBuildingName(e.getBuildingName().trim());
    e.setStatus(normalizeStatus(e.getStatus()));
  }

  private void normalize(SeatFlowFloor e) {
    e.setFloorName(e.getFloorName().trim());
    e.setStatus(normalizeStatus(e.getStatus()));
  }

  private void normalize(SeatFlowRoom e) {
    e.setRoomName(e.getRoomName().trim());
    e.setStatus(normalizeStatus(e.getStatus()));
  }
  ...
  private void validateStatus(String status) {
    if (!ENABLED.equals(status) && !DISABLED.equals(status))
      throw new ServiceException("状态只支持 enabled 或 disabled");
  }
