...
public class SeatFlowReportServiceImpl implements ISeatFlowReportService
{
    ...
    private static final Set<String> HEATMAP_METRICS = new HashSet<String>(Arrays.asList(
            METRIC_RESERVATION_COUNT, METRIC_USAGE_MINUTES));
    ...
    public List<SeatHeatmapVo> selectHeatmap(ReportQuery query)
    {
        ...
        validateMetric(query.getMetric(), HEATMAP_METRICS, "热力图指标只支持 reservation_count 或 usage_minutes");
    ...
    public List<RoomRankingVo> selectRoomRanking(ReportQuery query)
    {
        validateQuery(query);
        normalizeMetric(query, METRIC_RESERVATION_COUNT);
        validateMetric(query.getMetric(), ROOM_RANKING_METRICS, "自习室排行指标只支持 reservation_count、checkin_count 或 usage_rate");
